...
fn vec2_annotation(datatypes: &Vec<Datatype>) -> Variant {
    ...
    Variant::Vector2(Vector2::new(x_component, y_component))
}

fn vec2i16_annotation(datatypes: &Vec<Datatype>) -> Variant {
    let x_component = coerce_datatype_to_f32(datatypes.get(0), 0.0);
    let y_component = coerce_datatype_to_f32(datatypes.get(1), x_component);

    Variant::Vector2int16(Vector2int16::new(x_component as i16, y_component as i16))
}
...
fn vec3_annotation(datatypes: &Vec<Datatype>) -> Variant {
    ...
    Variant::Vector3(Vector3::new(x_component, y_component, z_component))
}

fn vec3i16_annotation(datatypes: &Vec<Datatype>) -> Variant {
    let x_component = coerce_datatype_to_f32(datatypes.get(0), 0.0);
    let y_component = coerce_datatype_to_f32(datatypes.get(1), x_component);
    let z_component = coerce_datatype_to_f32(datatypes.get(2), y_component);

    Variant::Vector3int16(Vector3int16::new(x_component as i16, y_component as i16, z_component as i16))
}

fn cframe_annotation(datatypes: &Vec<Datatype>) -> Variant {
    ...
    if let Some(Datatype::Variant(Variant::Float32(pos_x_component))) = datatypes_0 {
        ...
        return Variant::CFrame(CFrame::new(
...
fn numrange_annotation(datatype: &Vec<Datatype>) -> Variant {
    let min = coerce_datatype_to_f32(datatype.get(0), 0.0);
    let max = coerce_datatype_to_f32(datatype.get(1), min);
    
    return Variant::NumberRange(NumberRange::new(min, max))
}

fn content_annotation(datatypes: &Vec<Datatype>) -> Variant {
...
fn brickcolor_annotation(datatypes: &Vec<Datatype>) -> Variant {
    if let Some(Datatype::Variant(Variant::String(string))) = datatypes.get(0) {
        return Variant::BrickColor(BrickColor::from_name(string).unwrap_or(BrickColor::MediumStoneGrey))
    }

    Variant::BrickColor(BrickColor::MediumStoneGrey)
}
